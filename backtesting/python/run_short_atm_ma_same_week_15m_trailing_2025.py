...
IST_SUFFIX = "+05:30"
DAYWISE_FILENAME = "short_atm_ma_same_week_15m_trailing_2025_daywise.csv"
SUMMARY_FILENAME = "short_atm_ma_same_week_15m_trailing_2025_summary.md"
LOG_FILENAME = "short_atm_ma_same_week_15m_trailing_2025.log"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Backtest the 2025 same-week ATM short option strategy using a 15-minute trailing SMA stop.",
    )
    parser.add_argument(
        "--spot-file",
        type=Path,
...
def configure_logger(log_path: Path) -> logging.Logger:
    logger = logging.getLogger("short_atm_ma_same_week_15m_trailing_2025")
...
def resolve_trailing_ma_side_exit(
    ...
    for timestamp in day_timestamps[entry_index + 1 : exit_index]:
        ...
        # Recompute the prior-bar SMA on each 15-minute monitoring candle.
    ...
    try:
        for entry_date in trading_days:
            ...
            if ce_evaluation.signal_status == "TRADED":
                ce_exit = resolve_trailing_ma_side_exit(
            ...
            if pe_evaluation.signal_status == "TRADED":
                pe_exit = resolve_trailing_ma_side_exit(
...
def write_summary(results: List[TradeResult], output_path: Path, args: argparse.Namespace) -> None:
    ...
    lines: List[str] = [
        "# 2025 15-Minute Same-Week ATM MA Short Options Backtest With Trailing SMA Stop",
        "",
        "## Strategy Details",
        "",
