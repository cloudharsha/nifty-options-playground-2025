#!/usr/bin/env python3
"""
Heads & Tails — Parameter Grid Test, NIFTY Weekly (all data 2020–2026)

Same strategy as run_heads_tails_nifty.py, tested across every combination of:
  SL levels    : 20%, 30%, 40%, 50%
  Target levels: open (no booking), 60%, 70%, 80%, 90% decay
  = 20 combos × 5 random runs = 100 total series

"open" target = hold until SL triggers or day exit at 15:20 (no profit booking).

Day data loaded once; all combo × run simulations reuse the same contracts.
"""
...
IST_SUFFIX    = "+05:30"
BASE_FILENAME = "heads_tails_nifty_grid"
...
SL_LEVELS      = [0.20, 0.30, 0.40, 0.50]
TARGET_LEVELS: List[Optional[float]] = [None, 0.60, 0.70, 0.80, 0.90]   # None = open
SEEDS          = [42, 137, 2025, 777, 1984]
NUM_RUNS       = 5
BROK_PER_ORDER = 25.0
SLIP_PER_ORDER = 0.5
BROK_PER_TRADE = BROK_PER_ORDER * 2   # sell + buyback
...
class DayData:
    ...
    ce_noon_open: float        # 0.0 if missing
    pe_noon_open: float
...
class TradeResult:
    sl_pct: float
    target_pct: float          # -1.0 represents "open" (no target)
    combo: str
    run_number: int
    seed: int
    entry_date: str
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="Heads & Tails NIFTY parameter grid.")
...
# ---------------------------------------------------------------------------
# RESOLUTION (short option, parameterised SL + optional target)
# ---------------------------------------------------------------------------
...
def resolve_sell_leg(
    ...
    """
    Walk 1-min candles for a short option position.
    Returns (exit_ts, exit_price, reason, raw_points_pnl).
    raw_points_pnl is BEFORE slippage; slippage applied in apply_costs().

    target_pct=None → no profit target, hold until SL or day exit.

    Within each candle: gap checks first, then intra-candle SL before target
    (conservative assumption — SL adverse move noted before favourable move).
    """
    ...
    for ts in window:
        ...
        # gap conditions (open already through the level)
        if row.open_value >= sl_price:
            return ts, row.open_value, "gap_sl", entry_open - row.open_value
        if tgt_price is not None and row.open_value <= tgt_price:
            return ts, row.open_value, "gap_target", entry_open - row.open_value
        # intra-candle: SL checked before target
        if row.high_value >= sl_price:
            return ts, sl_price, "sl", entry_open - sl_price
        if tgt_price is not None and row.low_value <= tgt_price:
            return ts, tgt_price, "target", entry_open - tgt_price

    exit_row = contract.rows_by_timestamp.get(exit_ts)
    if exit_row:
        return exit_ts, exit_row.open_value, "day_close", entry_open - exit_row.open_value
    ...
    if candidates:
        ...
        return last_ts, last_row.open_value, "last_candle", entry_open - last_row.open_value

    return exit_ts, entry_open, "missing_exit", 0.0


def apply_costs(raw_pts: float, qty: int) -> Tuple[float, float]:
    """Returns (gross_pnl, net_pnl) after slippage and brokerage."""
...
def collect_day_data(args: argparse.Namespace) -> Tuple[List[DayData], List[SkippedDay]]:
    ...
    try:
        for entry_date in trading_days:
            ...
            logger.debug("LOADED %s atm=%s ce=%.2f pe=%.2f", entry_date, atm,
...
def simulate_one(
    ...
    for dd in days:
        ...
        m_exit_ts, m_exit_px, m_reason, m_raw_pts = resolve_sell_leg(
        ...
        if do_second:
            s_exit_ts, s_exit_px, s_exit_reason, s_raw_pts = resolve_sell_leg(
...
def _avg_net(results_map: ResultsMap, sl: float, tgt: Optional[float]) -> float:
    runs = results_map.get((sl, tgt), {})
    if not runs:
        return 0.0
    return sum(sum(r.total_net_pnl for r in v) for v in runs.values()) / len(runs)


def _avg_cagr(results_map: ResultsMap, sl: float, tgt: Optional[float],
              capital: float) -> float:
    runs = results_map.get((sl, tgt), {})
    if not runs:
        return 0.0
    ...
    for rr in runs.values():
        ...
        cagr = compute_cagr(net, capital, rr[0].entry_date, rr[-1].entry_date)
        cagrs.append(cagr * 100)
    return sum(cagrs) / len(cagrs) if cagrs else 0.0


def _avg_dd(results_map: ResultsMap, sl: float, tgt: Optional[float]) -> float:
    runs = results_map.get((sl, tgt), {})
    if not runs:
        return 0.0
...
def _avg_winpct(results_map: ResultsMap, sl: float, tgt: Optional[float]) -> float:
    runs = results_map.get((sl, tgt), {})
    if not runs:
        return 0.0
    wps = []
    for rr in runs.values():
        if rr:
            wps.append(sum(1 for r in rr if r.total_net_pnl > 0) / len(rr) * 100)
    return sum(wps) / len(wps) if wps else 0.0


def _positive_runs(results_map: ResultsMap, sl: float, tgt: Optional[float]) -> int:
    runs = results_map.get((sl, tgt), {})
...
def write_summary(
    ...
    # Derive first/last date from any run
    ...
    lines: List[str] = [
        ...
        "## Strategy",
        "",
        f"- Entry `{ENTRY_TIME}` — sell ATM CE or PE at random (50/50)",
        ...
        f"- Period: `{first_d}` → `{last_d}` | Days loaded: `{num_days}` "
    ...
    # ── GRID 5: How many of 5 runs are profitable ─────────────────────────
    lines += [
    ...
    for sl, sl_lbl in zip(SL_LEVELS, sl_labels):
        cells = [str(_positive_runs(results_map, sl, tgt)) + "/5" for tgt in TARGET_LEVELS]
        lines.append(f"| SL={sl_lbl} | " + " | ".join(f"`{c}`" for c in cells) + " |")
    lines += ["", "---", ""]
    ...
    for rank, (avg_net, sl, tgt) in enumerate(ranked, 1):
        ...
        pos   = _positive_runs(results_map, sl, tgt)
    ...
    # ── PER-RUN DETAIL FOR BEST COMBO ─────────────────────────────────────
    best_ck    = combo_key(best_sl, best_tgt)
    ...
    for month in all_months:
        ...
        avg_m = sum(
            sum(r.total_net_pnl for r in best_runs[run_id] if r.entry_date[:7] == month)
    ...
    lines += [
        f"### Yearly P/L — Best Combo (each run + avg)",
        "",
    ...
    for year in all_years:
        ...
        avg_y = sum(
            sum(r.total_net_pnl for r in best_runs[run_id] if r.entry_date[:4] == year)
            for run_id in best_runs
        ) / len(best_runs)
    ...
    # ── YEARLY TABLE: avg P/L per combo per year ──────────────────────────
    all_years2 = sorted({r.entry_date[:4] for r in sample_rr})
    all_combos_ordered = [(sl, tgt) for sl in SL_LEVELS for tgt in TARGET_LEVELS]
    combo_labels_short = [f"SL{int(sl*100)}T{target_label(tgt)}" for sl, tgt in all_combos_ordered]
    ...
    for year in all_years2:
        cells = []
        for sl, tgt in all_combos_ordered:
            runs_for_combo = results_map.get((sl, tgt), {})
            if not runs_for_combo:
                cells.append("—")
                continue
            avg_y = sum(
                sum(r.total_net_pnl for r in runs_for_combo[run_id] if r.entry_date[:4] == year)
                for run_id in runs_for_combo
            ) / len(runs_for_combo)
