...
IST_SUFFIX = "+05:30"
DAYWISE_FILENAME = "short_atm_nifty_ma_weekly_overnight_hedged_2025_daywise.csv"
SUMMARY_FILENAME = "short_atm_nifty_ma_weekly_overnight_hedged_2025_summary.md"
LOG_FILENAME = "short_atm_nifty_ma_weekly_overnight_hedged_2025.log"
...
class TradeResult:
    ...
    hedge_strike: str
    hedge_contract_name: str
    hedge_option_entry_timestamp: str
    hedge_option_entry_open: str
    hedge_option_exit_timestamp: str
    hedge_option_exit_open: str
    gross_pnl: str
    brokerage: str
    net_pnl: str
    remarks: str
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Backtest the 2025 overnight weekly directional ATM short option strategy using a NIFTY 25-SMA signal with a fixed same-side hedge.",
    )
    parser.add_argument(
        "--spot-file",
        type=Path,
    ...
    parser.add_argument("--hedge-distance", type=int, default=200)
...
def configure_logger(log_path: Path) -> logging.Logger:
    logger = logging.getLogger("short_atm_nifty_ma_weekly_overnight_hedged_2025")
...
def hedge_strike_for_side(atm_strike: int, sold_side: str, hedge_distance: int) -> int:
    if sold_side == "CE":
        return atm_strike + hedge_distance
    return atm_strike - hedge_distance
...
def make_result(
    ...
    return TradeResult(
        ...
        hedge_contract_name=hedge_contract_name,
        hedge_option_entry_timestamp=hedge_option_entry_timestamp,
        hedge_option_entry_open=hedge_option_entry_open,
        hedge_option_exit_timestamp=hedge_option_exit_timestamp,
        hedge_option_exit_open=hedge_option_exit_open,
...
def run_backtest(args: argparse.Namespace) -> List[TradeResult]:
    ...
    try:
        for entry_date in trading_days:
            ...
            hedge_strike = hedge_strike_for_side(atm_strike, sold_side, args.hedge_distance)
            hedge_strike_text = str(hedge_strike)
            sold_contract_path = args.options_dir / expiry_date / f"NIFTY_{atm_strike}_{sold_side}_{option_suffix}.csv"
            hedge_contract_path = args.options_dir / expiry_date / f"NIFTY_{hedge_strike}_{sold_side}_{option_suffix}.csv"

            sold_contract = load_contract(sold_contract_path, contract_cache)
            hedge_contract = load_contract(hedge_contract_path, contract_cache)
            if sold_contract is None or hedge_contract is None:
                ...
                if hedge_contract is None:
                    missing_names.append(hedge_contract_path.name)
                result = make_result(
                    entry_date=entry_date,
                    status="SKIPPED",
                    ...
                    hedge_strike=hedge_strike_text,
                    hedge_contract_name=hedge_contract_path.name,
                    remarks=f"Missing option file(s): {', '.join(missing_names)}",
                )
                results.append(result)
                logger.info(
                    "SKIPPED date=%s expiry=%s side=%s strike=%s hedge=%s reason=%s",
                    entry_date,
                    expiry_date,
                    sold_side,
                    strike_text,
            ...
            hedge_entry_row = hedge_contract.rows_by_timestamp.get(option_entry_timestamp)
            hedge_exit_row = hedge_contract.rows_by_timestamp.get(option_exit_timestamp)
            ...
            if hedge_entry_row is None:
                missing_points.append(f"{hedge_contract_path.name} missing entry timestamp {option_entry_timestamp}")
            if hedge_exit_row is None:
                missing_points.append(f"{hedge_contract_path.name} missing exit timestamp {option_exit_timestamp}")
            ...
            if missing_points:
                ...
                result = make_result(
                    ...
                    hedge_strike=hedge_strike_text,
                    hedge_contract_name=hedge_contract_path.name,
                    hedge_option_entry_timestamp=option_entry_timestamp,
                    hedge_option_exit_timestamp=option_exit_timestamp,
                    remarks=remarks,
                )
                results.append(result)
                logger.info(
            ...
            sold_leg_pnl = leg_pnl_after_slippage(
                sold_entry_row.open_value - sold_exit_row.open_value,
                args.slippage_points_per_order,
            )
            hedge_leg_pnl = leg_pnl_after_slippage(
                hedge_exit_row.open_value - hedge_entry_row.open_value,
                args.slippage_points_per_order,
            )
            gross_pnl = (sold_leg_pnl + hedge_leg_pnl) * contract_multiplier
            ...
            result = make_result(
                ...
                hedge_contract_name=hedge_contract_path.name,
                hedge_option_entry_timestamp=option_entry_timestamp,
                hedge_option_entry_open=hedge_entry_row.open_text,
                hedge_option_exit_timestamp=option_exit_timestamp,
                hedge_option_exit_open=hedge_exit_row.open_text,
                gross_pnl=gross_pnl,
                brokerage=brokerage,
                net_pnl=net_pnl,
            )
            results.append(result)
            logger.info(
                "TRADED date=%s expiry=%s side=%s strike=%s hedge=%s gross=%s brokerage=%s net=%s",
                entry_date,
                expiry_date,
                sold_side,
                strike_text,
...
def write_daywise_csv(results: List[TradeResult], output_path: Path) -> None:
    fieldnames = [
        ...
        "hedge_strike",
        "hedge_contract_name",
        "hedge_option_entry_timestamp",
        "hedge_option_entry_open",
        "hedge_option_exit_timestamp",
        "hedge_option_exit_open",
        "gross_pnl",
        "brokerage",
        "net_pnl",
        "remarks",
...
def write_summary(results: List[TradeResult], output_path: Path, args: argparse.Namespace) -> None:
    ...
    lines: List[str] = [
        ...
        f"- Hedge distance: {args.hedge_distance} points on the same option side",
        ...
        (
            ...
            f"so one completed hedged spread pays Rs {int(args.brokerage_per_order * 4)}"
        ),
        "",
        "## Results Summary",
        "",
        ...
        f"- CE-spread count: `{ce_sell_count}`",
        f"- PE-spread count: `{pe_sell_count}`",
