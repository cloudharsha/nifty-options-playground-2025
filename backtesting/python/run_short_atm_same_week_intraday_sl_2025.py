...
IST_SUFFIX = "+05:30"
BASE_FILENAME = "short_atm_same_week_intraday_sl_2025"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Backtest a 2025 intraday same-week ATM short straddle with fixed leg stop losses.",
    )
    parser.add_argument(
        "--spot-file",
        type=Path,
    ...
    parser.add_argument("--stop-loss-points", type=float, default=50.0)
    ...
    if args.stop_loss_points <= 0:
        parser.error("--stop-loss-points must be positive")
    if args.lot_size <= 0:
...
def empty_leg(side: str, contract_file: str = "", remarks: str = "") -> LegResolution:
...
def resolve_leg(
    ...
    if entry_row is None:
        remarks = (
            ...
            else f"{contract.path.name} missing entry timestamp {entry_timestamp}"
        )
        return LegResolution(
            side=side,
    ...
    stop_price = entry_row.open_value + stop_loss_points
    ...
    for timestamp in day_timestamps[entry_index:exit_index]:
        ...
        if row is None:
            return LegResolution(
                ...
                remarks=f"{contract.path.name} missing monitoring timestamp {timestamp}",
            )
    ...
    if exit_row is None:
        return LegResolution(
            ...
            remarks=f"{contract.path.name} missing scheduled exit timestamp {exit_timestamp}",
        )
            ...
            for entry_date in trading_days:
                ...
                if ce_contract is None or pe_contract is None:
                    ...
                    result = make_day_result(
                        ...
                        ce_leg=empty_leg("CE", ce_path.name if ce_contract is None else ce_contract.path.name),
                        pe_leg=empty_leg("PE", pe_path.name if pe_contract is None else pe_contract.path.name),
...
def write_summary(results: List[DayResult], output_path: Path, args: argparse.Namespace) -> None:
    ...
    lines: List[str] = [
        ...
        f"- Stop rule: independent `{format_number(args.stop_loss_points)}` point premium stop per short leg",
