...
Strategy rules (same as expiry-included variant):
  - Entry 09:20, exit 15:20.
  - Sell ATM CE + PE; 20% independent SL per leg.
  - Always trade current-week expiry, even on expiry day itself.
  - No balance filter.
  - NIFTY on Monday / Tuesday / Friday  (~300 qty, strike rounding 50).
  - SENSEX on Wednesday / Thursday      (~100 qty, strike rounding 100).
"""
...
IST_SUFFIX    = "+05:30"
BASE_FILENAME = "combined_nifty_sensex_expiry_incl_2025"
...
def get_nifty_lot_config(expiry_date: str) -> Tuple[int, int]:
    ...
    if d <= datetime.date(2021, 10, 6):  return 75, 4   # 300
    if d <= datetime.date(2024, 4, 25):  return 50, 6   # 300
    if d <= datetime.date(2024, 11, 21): return 25, 12  # 300
    if d <= datetime.date(2025, 12, 30): return 75, 4   # 300
    return 65, 5                                         # 325
...
def load_spot_opens(spot_file: Path, entry_time: str) -> Tuple[List[str], Dict[str, Tuple[float, str]]]:
    """Return (sorted trading days, {date: (open_float, open_str)} at entry_time)."""
    ...
    try:
        for entry_date in nifty_days:
            ...
            if index == "NIFTY":
                ...
                lot_size, num_lots = 10, 10   # 100 qty fixed for SENSEX
                atm = round_to_100(spot_val)

            qty    = lot_size * num_lots
            suffix = expiry_suffix(expiry_date)
            ce_path = options_dir / expiry_date / f"{prefix}_{atm}_CE_{suffix}.csv"
            pe_path = options_dir / expiry_date / f"{prefix}_{atm}_PE_{suffix}.csv"
            ...
            if missing:
                results.append(make_skip(index, entry_date, day_name, "missing_contract_file",
                                         f"Missing: {', '.join(missing)}",
                                         expiry_date, entry_ts, spot_text,
                                         str(atm), str(lot_size), str(num_lots), str(qty)))
                logger.info("SKIPPED %s date=%s missing_contract atm=%s", index, entry_date, atm)
                continue
            ...
            if missing_ts:
                results.append(make_skip(index, entry_date, day_name, "missing_entry_candle",
                                         f"No {entry_ts} candle in: {', '.join(missing_ts)}",
                                         expiry_date, entry_ts, spot_text,
                                         str(atm), str(lot_size), str(num_lots), str(qty)))
                logger.info("SKIPPED %s date=%s missing_entry_candle atm=%s", index, entry_date, atm)
                continue
            ...
            if ce_open <= 0 or pe_open <= 0:
                results.append(make_skip(index, entry_date, day_name, "zero_price",
                                         f"CE={fmt(ce_open)} PE={fmt(pe_open)}",
                                         expiry_date, entry_ts, spot_text,
                                         str(atm), str(lot_size), str(num_lots), str(qty)))
                logger.info("SKIPPED %s date=%s zero_price", index, entry_date)
                continue
            ...
            logger.info(
                "TRADED %s date=%s day=%s expiry=%s atm=%s qty=%s ce=%.2f pe=%.2f sl=[%s] net=%.2f",
                index, entry_date, day_name, expiry_date, atm, qty,
...
def write_daywise_csv(results: List[TradeResult], output_path: Path) -> None:
    fieldnames = [
        ...
        "expiry_date", "spot_entry_timestamp", "spot_entry_open", "atm_strike",
...
def compute_equity_curve(net_pnls: List[float]) -> Tuple[float, float]:
    """Return (max_drawdown, max_cumulative_profit)."""
