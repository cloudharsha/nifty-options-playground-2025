...
with a 2-SL-per-day cap.

Changes from the 0920_random baseline:
  - Same entry at 09:20 with fallthrough to 09:30 if option data is unavailable.
  - Same random day-skipping at 30%, 40%, 50% with the same seeds.
  - NEW: if stop-loss fires twice in a single day, trading halts for the rest of that day.
    This is the only difference vs the 0920_random baseline.

Extra metrics vs baseline:
  - Max consecutive SL exits within a single day
  - Max consecutive SL exits across the entire backtest
  - Days where trading stopped early due to the 2-SL cap

5 independent runs per skip rate (15 total), fixed seeds: 42, 137, 999, 2024, 31415.
"""
...
IST_SUFFIX = "+05:30"
SUMMARY_FILENAME = "short_atm_nifty_ma_weekly_intraday_trailing_0920_random_2slcap_summary.md"
LOG_FILENAME = "short_atm_nifty_ma_weekly_intraday_trailing_0920_random_2slcap.log"
...
# Stop trading for the rest of the day after this many SL exits
SL_CAP_PER_DAY = 2
...
class DayResult:
    ...
    sl_capped: bool   # True if trading stopped early due to 2-SL cap
    remarks: str
...
class SimResult:
    ...
    days_2sl_capped: int   # days where the 2-SL cap was triggered
    first_day: str
    last_day: str
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="09:20-start random-skip backtest with 2-SL/day cap.",
    )
    ...
    parser.add_argument("--results-dir", type=Path,
                        default=repo_root / "backtesting" / "results" / "25ma-intraday" / "with-sl-cap")
...
def configure_logger(log_path: Path) -> logging.Logger:
    logger = logging.getLogger("short_atm_nifty_ma_weekly_intraday_trailing_0920_random_2slcap")
...
def run_single_simulation(
    ...
    for day_index, entry_date in enumerate(spot_15m.trading_days):
        ...
        stop_day = False
        daily_sl_count = 0   # SL exits so far today
        ...
        for entry_ts in entry_timestamps:
            ...
            if outcome.exit_reason == "stop_loss_ma_touch":
                ...
                if daily_sl_count >= SL_CAP_PER_DAY:
                    # 2-SL cap reached — halt trading for the rest of this day
...
def write_summary(sim_results: List[SimResult], path: Path, args: argparse.Namespace) -> None:
    ...
    lines: List[str] = [
        ...
        f"- **NEW — Daily SL cap**: if stop-loss fires {SL_CAP_PER_DAY} times in a day, no further entries that day",
        "",
        "## Baseline (full participation, 09:30 start, no SL cap)",
        "",
        "| Metric | Value |",
        "|--------|-------|",
        f"| Net P/L | {fmt_inr(baseline_net)} |",
        f"| CAGR | {baseline_cagr:.2f}% |",
        f"| Max Drawdown | {fmt_inr(baseline_dd)} |",
        "| Capital base | Rs 10,00,000 |",
        "",
        "## Simulation Results",
        "",
        "| Skip | Run | Seed | Traded Days | Rnd Skip | Strat Skip | 2SL-Cap Days"
    ...
    lines.append(
        ...
        " | Avg Max DD | Avg Win% | Avg Traded Days | Avg 2SL-Cap Days | Avg Max SL/Day | Avg Max SL Overall |"
    )
    lines.append(
        "|-----------|-------------|----------|----------|----------"
        "|------------|----------|-----------------|------------------|----------------|---------------------|"
    ...
    for rate in SKIP_RATES:
        ...
        avg_capped = sum(r.days_2sl_capped for r in group) / len(group)
        ...
        lines.append(
            ...
            f"| {avg_win:.1f}% | {avg_td:.0f} | {avg_capped:.0f} "
            f"| {avg_sl_day:.1f} | {avg_sl_overall:.1f} |"
        )

    lines.extend(["", "## SL Streak Detail", ""])
    lines.append("| Skip | Run | Max SL in a Single Day | Max SL Streak (All Trades) | Total Stop Exits | Total Trades | 2SL-Cap Days |")
    lines.append("|------|-----|------------------------|----------------------------|------------------|--------------|--------------|")
    for r in sim_results:
        ...
        lines.append(
            ...
            f"| {r.max_consec_sl_in_day} | {r.max_consec_sl_overall} "
            f"| {r.stop_exits} | {total_trades} | {r.days_2sl_capped} |"
        )
...
def main() -> None:
    ...
    for skip_rate in SKIP_RATES:
        for run_idx in range(1, NUM_RUNS + 1):
            ...
            write_tradewise_csv(all_trade_results, args.results_dir / f"short_atm_nifty_ma_weekly_intraday_trailing_0920_random_2slcap_{tag}_trades.csv")
            write_daywise_csv(day_results, args.results_dir / f"short_atm_nifty_ma_weekly_intraday_trailing_0920_random_2slcap_{tag}_daywise.csv")

            logger.info(
                "DONE skip=%.0f%% run=%d seed=%d traded=%d net=%.2f cagr=%.2f%% sl_day=%d sl_overall=%d 2sl_capped=%d",
                skip_rate * 100, run_idx, seed,
                sr.traded_days, sr.net_pnl, sr.cagr,
                sr.max_consec_sl_in_day, sr.max_consec_sl_overall, sr.days_2sl_capped,
            )
            print(
                f"         traded={sr.traded_days}  net={fmt_inr(sr.net_pnl)}"
                f"  CAGR={sr.cagr:.2f}%  SL/day={sr.max_consec_sl_in_day}"
                f"  SL_total={sr.max_consec_sl_overall}  2SL_capped={sr.days_2sl_capped}"
            )
